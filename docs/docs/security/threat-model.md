...
| **Repudiation** — disputed login/action | Audit row on create/update/delete of audited models, with actor, IP and timestamp, written in the same transaction as the change; a failed audit write fails the change (`packages/db/src/audit.extension.ts`, v0.7.1). Before v0.7.1 most such writes left no row, and rows that were written had no actor — that history can't be recovered | UPDATE rows record the new values only, not the old ones (before = null); bulk writes (updateMany, createMany, deleteMany, upsert) on audited models, role-permission changes among them, write no row (both tracked in docs/todo.md). Audit log itself is append-managed by app code, not DB-trigger-enforced like financial ledger rows — a compromised app-level credential could in theory suppress a specific audit write. Financial actions are separately protected by the ledger's own DB-trigger immutability. |
