...
function ChangePasswordCard() {
  ...
  const mutation = useMutation({
    mutationFn: (body: { currentPassword: string; newPassword: string }) =>
      api('/portal/auth/change-password', { method: 'POST', body: JSON.stringify(body) }),
  });
...
function TwoFactorCard() {
  ...
  const setupMutation = useMutation({
    mutationFn: () => api<{ secret: string; otpauthUrl: string }>('/portal/auth/totp/setup', { method: 'POST' }),
  });
  const confirmMutation = useMutation({
    mutationFn: (body: { code: string }) =>
      api<{ recoveryCodes: string[] }>('/portal/auth/totp/confirm', {
        method: 'POST',
        body: JSON.stringify(body),
      }),
  });
  const disableMutation = useMutation({
    mutationFn: () => api('/portal/auth/totp/disable', { method: 'POST' }),
  });
  ...
  const confirm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSetup(null);
      setCode('');
      qc.invalidateQueries({ queryKey: ['portal', 'auth', 'me'] });
    } catch (err) {
      setError((err as Error).message);
    }
  };
    ...
    <div className="rounded-lg border border-slate-200 bg-white p-4">
      ...
      {recoveryCodes && (
        <div className="mt-3 rounded-md bg-amber-50 border border-amber-200 p-3 text-sm">
          ...
          <button onClick={() => setRecoveryCodes(null)} className="mt-3 text-sm font-medium text-amber-800 underline">
            Done
          </button>
        </div>
      )}
      ...
      {!recoveryCodes && me?.totpEnabled && (
        <div className="mt-3">
          ...
          <button
            ...
            className="mt-3 rounded-md border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 shadow-sm disabled:opacity-50"
          >
      ...
      {!recoveryCodes && !me?.totpEnabled && !setup && (
        <div className="mt-3">
          ...
          <button
            ...
            className="mt-3 rounded-md bg-blue-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm disabled:opacity-50"
          >
