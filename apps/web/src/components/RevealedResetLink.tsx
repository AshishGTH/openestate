...
export default function RevealedResetLink({ url, expiresAt, onDismiss }: RevealedResetLinkProps) {
  ...
  // so a stale setState never fires after unmount.
  useEffect(() => {
    if (!copied) return;
  ...
  return (
    <div className="mt-4 rounded-lg border border-amber-300 bg-amber-50 p-4">
      ...
      <div className="mt-2 flex items-center gap-2">
        <code className="flex-1 select-all break-all rounded bg-white border border-amber-300 px-3 py-2 text-sm font-mono">
          {url}
        </code>
        <button
          onClick={() => {
